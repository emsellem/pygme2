...
class dynMGE(object) :
    ...
    def _IntMu1(self, T, R2, Z2, ilist=None) :
        ...
        for j in range(self.nStarGauss) :
            expfac = self._pParam.qParc[j] * exp(- (R2 + Z2 / facdenom[j]) * T2 / self._dParam.dsig3d2[j]) / sqrt(facdenom[j])
            for i in ilist :
    ...
    def _IntMu2(self, T, X2, Y2, cosi2, sini2, ilist=None) :
        ...
        T2Bij_soft = 1. - self._dParam.Bij_soft * T2
        facdenom = 1. - self.e2 * T2
        e2T4 = T4 * self.e2 / (self._pParam.dsig3d2 * facdenom)

        Integrand = np.zeros_like(X2)
        for j in range(self.n_gaussians) :
            for i in ilist :
                A = T2 / self._dParam.dsig3d2[j] + 1. / self._pParam.dsig3d2[i]  # in arcsec-2
                B = e2T4[j] + self.e2q2dsig3d2[i]              # in arcsec-2
                ...
                num = self._dParam.q2sig3d2[i] + X2 * sini2 * (self.e2[i] - T2 * self.e2[j]) # in arcsec^2
    ...
    def _Mu2(self, X, Y, inclin=90., ilist=None) :
        ...
        X2 = X * X
        Y2 = Y * Y
        self.rhop = np.sum(self.imax2d[ilist] * exp(- (X2[...,np.newaxis] + Y2[...,np.newaxis] / self.q2d2[ilist]) / self.dsig2d2[ilist]))
    ...
    def _IntPot(self, T, R2, Z2, ilist=None) :
        ...
        Integrand = self._dParam.sig3d2[ilist] * self._pParam.qParc[ilist] * exp(- (R2[...,np.newaxis] + Z2[...,np.newaxis] / denom) * T2 / self._dParam.dsig3d2[ilist]) / sqrt(denom)
    ...
    def Potential(self, R, Z, ilist=None) :
        ...
        :param ilist: list of indices for the Gaussians to consider (0 to n_gaussians-1)
    ...
    def _IntVc(self, T, R2, ilist=None) :
        ...
        denom = 1. - self.e2 * T2
        for i in ilist :
            Integrand += self._pParam.qParc[i] * exp(- R2 * T2 / self._dParam.dsig3d2[i]) / sqrt(denom[i])
        return Integrand * T2
    ...
    def Vcirc(self, R, ilist=None) :
        ...
        return sqrt(result * self._PIG)  # en km.s-1
    ### ==============================================================================
    ...
    def _IntaccR(self, T, R2, Z2, ilist=None) :
        ...
        denom = 1. - self.e2 * T2
        for i in ilist :
            Integrand += self._pParam.qParc[i] * exp(- (R2 + Z2 / denom[i]) * T2 / self._dParam.dsig3d2[i]) / sqrt(denom[i])
        return Integrand * T2
    #===========================================================
    ...
    def _IntaccZ(self, T, R2, Z2, ilist=None) :
        ...
        denom = 1. - self.e2 * T2
        for i in ilist :
            Integrand += self._pParam.qParc[i] * exp(- (R2 + Z2 / denom[i]) * T2 / self._dParam.dsig3d2[i]) / (denom[i])**(1.5)
        return Integrand * T2
    #===========================================================
    ...
    def _accR(self, R, Z, ilist=None) :
        ...
        return self._PIG * result * R / self._pc_per_arcsec  # en km^2.s-2.pc-1
    #===========================================================
    ...
    def _accZ(self, R, Z, ilist=None) :
        ...
        return self._PIG * result * Z / self._pc_per_arcsec  # en km^2.s-2.pc-1
    #===========================================================
    ##################################################################
    ...
    def _Intd2Potd2R(self, T, R2, Z2, ilist=None) :
        ...
        denom = 1. - self.e2 * T2
        for i in ilist :
            Integrand += self._pParam.qParc[i] * exp(- (R2 + Z2 / denom[i]) * T2 / self._dParam.dsig3d2[i]) * (1. - R2 * T2 / self._dParam.sig3d2[i]) / sqrt(denom[i])
        return Integrand * T2
    #===========================================================
    #######################################################
    def _d2Potd2R(self, R, Z, ilist=None) :
        ...
        return self._PIG * result / (self._pc_per_arcsec*self._pc_per_arcsec)  # en km^2.s-2.pc-2
    #===========================================================
    #######################################################
    ...
    def _FluxDensity(self, R2, Z2, ilist=None) :
        ...
        rhoL = np.zeros((self.n_gaussians, len(R2)), np.float32)
    ...
    def _MassDensity(self, R2, Z2, ilist=None) :
        ...
        """
        ### Compute .rho and .rhoT the individual and total M density on the grid
        rho = np.zeros((self.n_gaussians, len(R2)), np.float32)
    ...
    def Omega(self, R, ilist=None) :
        ...
        return sqrt(self._PIG * result) / self._pc_per_arcsec  # en km.s-1.pc-1
    ### ==============================================================================
    ##################################################################
    ...
    def _Intkappa(self, T, R2, ilist=None) :
        ...
        denom = 1. - self.e2 * T2
        for i in ilist :
            Integrand += self._pParam.qParc[i] * exp(- R2 * T2 / self._pParam.dsig3d2[i]) * (4. - R2 * T2 / self._dParam.sig3d2[i]) / sqrt(denom[i])
        return Integrand * T2
    #===========================================================
    #######################################################
    ...
    def QToomre(self, R, Zcut, ilist=None) :
        ...
        ## So QT in pc-1 * arcsec, so we multiply by _pc_per_arcsec
        QT = k * SigmaR * self._pc_per_arcsec / (3.36 * self.G * SurfDensity)
    ...
    def _intsigma_z2(self, T, R2, Z2, ilist=None) :
        ...
        T2 = T * T
        T2Bij_soft = 1. - self._dParam.Bij_soft * T2
        q2sig3d2T2 = self._dParam.q2sig3d2 * T2
        Integrand = np.zeros_like(R2)        # this has the dimension of the particules array
        denom = 1. - self.e2 * T2
        expfac = self._pParam.qParc * exp(- (R2[...,np.newaxis] + Z2[...,np.newaxis] / denom) * T2 / self._dParam.dsig3d2) / sqrt(denom)
        for j in range(self.n_gaussians) :
#         expfac = self._pParam.qParc[j] * exp(- (R2 + Z2 / denom[j]) * T2 / self._pParam.dsig3d2[j]) / sqrt(denom[j])
#         Integrand = Integrand + np.sum(self.rho * self._dParam.q2sig3d2 * expfac / T2Bij_soft[:,j], axis=-1)
            Integrand += np.sum(self.rho[ilist].transpose() * (expfac[...,j])[...,np.newaxis] * q2sig3d2T2[ilist] / T2Bij_soft[ilist,j], axis=-1)
...
#      T2e2j = T2 * self.e2
        ...
        expfac = qParcT2 * exp(- (R2[...,np.newaxis] + Z2[...,np.newaxis] / denom) * T2 / self._pParam.dsig3d2) / sqrt(denom)
        for j in range(self.n_gaussians) :
            for i in ilist :
#            Integrand += self.rho[i] * (R2 * (self.e2[i] - T2e2j[j]) + self._dParam.q2sig3d2[i]) * expfac[...,j] / T2Bij_soft[i,j]
                Integrand += self.rho[i] * (R2 * (self._dParam.mkRZ2q2[i] - self._dParam.Dij_soft[i,j] * T2) + self._dParam.kRZ2[i] \
                                 * self._dParam.q2sig3d2[i]) * expfac[...,j] / T2Bij_soft[i,j]
    ...
    def _sigma_z2_fromR2Z2(self, R2, Z2, ilist=None) :
        ...
        if self.Mbh > 0. :
            for i in ilist:
                var = (r / self._pParam.dqsig3d[i]).astype(floatG)
        ...
        return sigz2 * self._PIG / self.rhoT
    #===========================================================
    #######################################################
    ...
    def _vtheta2_fromR2Z2(self, R2, Z2, ilist=None) :
        ...
        if self.Mbh > 0. :
            for i in ilist :
                ...
                lasterm[mask] = self._dParam.sqpi2s[i] * special.erfc(var[mask]) * np.exp(var[mask]**2)
                lasterm[~mask] = 2. / (r[~mask] + sqrt(r2[~mask] + self._dParam.qq2s2[i]))
                VT2 += (1. + self._dParam.e2q2sig3d2[i] * R2) * self.rho[i] * self.facMbh \
                          * (1. / rsoft - lasterm)  # in rhoT * M arcsec pc-2 / 4 PI G

        return VT2 * self._PIG / self.rhoT
    #===========================================================
    #######################################################
    ...
    def _sigmaz2_muTheta2_fromR2Z2(self, R2, Z2, ilist=None) :
        ...
        if self.Mbh > 0. :
            for i in ilist :
                ...
                lasterm[~mask] = 2. / (r[~mask] + sqrt(r2[~mask] + self._dParam.qq2s2[i]))
                sigz2 += self.rho[i] * self.facMbh * (1. / rsoft - lasterm) # in rho * M arcsec pc-2 / 4 PI G
                muTheta2 += (1. + self._dParam.e2q2sig3d2[i] * R2) * self.rho[i] * self.facMbh \
                         * (1. / rsoft - lasterm)  # in rhoT * M arcsec pc-2 / 4 PI G

        sigz2 *= self._PIG / self.rhoT
        muTheta2 *= self._PIG / self.rhoT
