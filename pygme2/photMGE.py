...
class photMGE(object):
    ...
    def _rho3D_1G_fromR2Z2(self, R2, Z2, ind) :
        ...
        if self._model3d is not None : 
            print "No Spatial Gaussians yet"
            return R2*0.
        return self.Parc[ind] * exp(- (R2 + Z2 / self.qzx2[ind]) / self._pParam.dsig3d2[ind])  # in Mass/pc-2/arcsec-1
    ...
    def _rho2D_1G_fromX2Y2(self, X2, Y2, ind) :
        ...
        if self._findGauss2D == 0 : 
            print "No projected Gaussians yet"
            return X2*0.
        return self.Pp[ind] * exp(- (X2 + Y2 / self.q2d2[ind]) / self.dsig2d2[ind])  # in Mass/pc-2
    ###=================================================================
    ##################################################################
    ### Derive the spatial and projected densities
    ##################################################################
    ...
    def _rhoL3D_1G_fromR2Z2(self, R2, Z2, ind) :
        ...
        if self._model3d is not None : 
            print "No Spatial Gaussians yet"
            return R2*0.
        return self.imax3d[ind] * exp(- (R2 + Z2 / self.qzx2[ind]) / self._pParam.dsig3d2[ind])  # I in Lum.pc-2/arcsec-1
    ### Deriving the spatial luminosity density for 1 gaussian
    ...
    def _rhoL2D_1G_fromX2Y2(self, X2, Y2, ind) :
        ...
        if self._findGauss2D == 0 : 
            print "No projected Gaussians yet"
            return X2*0.
        return self.imax2d[ind] * exp(- (X2 + Y2 / self.q2d2[ind]) / self.dsig2d2[ind])  # I in Lum.pc-2
    ###=============================================================================
    ### Deriving the projected luminosity density for 1 gaussian
    ...
    def FluxSurf_1G(self, R, ind) :
        ...
        if self._model3d is not None : 
            ...
            return R * 0.
        R2 = R * R
        return self.imax3d[ind] * sqrt(2. * np.pi) * self.qsig3d[ind] * exp(- R2 / self._pParam.dsig3d2[ind])  # in Lum.pc-2
    ...
    def _rhointLZ_1G(self, R, Zcut, ind):
        ...
        if self._model3d is not None : 
            print "No Spatial Gaussians yet"
            return R * 0.
        return self.FluxSurf_1G(R, ind) * np.float32(special.erf(Zcut / self._pParam.dqsig3d[ind]))  # in Lum.pc-2
    ...
    def MassSurf1(self, R, ind) :
        ...
        if self._model3d is not None : 
            ...
            return R * 0.
        R2 = R * R
        return self.Parc[ind] * sqrt(2. * np.pi) * self.qsig3d[ind] * exp(- R2 / self._pParam.dsig3d2[ind])  # in Mass.pc-2
    ...
    def rhointMZ1(self, R, Zcut, ind) :
        ...
        if self._model3d is not None : 
            print "No Spatial Gaussians yet"
            return R * 0.
        return self.MassSurf1(R, ind) * float(special.erf(Zcut / self._pParam.dqsig3d[ind]))  # in Mass.pc-2
